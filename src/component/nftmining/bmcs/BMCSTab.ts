...
import BMCSItem from "./BMCSItem";

export default class BMCSTab extends DomNode {
                        ...
                        if (owner !== undefined) {
                            ...
                            if (balance.lt(fee)) {
                                ...
                                if ((await MixContract.allowance(owner, BiasPoolContract.address)).lt(fee)) {
                                    ...
                                    setTimeout(async () => {
                                        await BiasPoolContract.claim(this.mates);
                                    }, 2000);
                                } else {
                                    await BiasPoolContract.claim(this.mates);
                                }
        ...
        if (walletAddress !== undefined) {
            ...
            for (let i = 0; i < balance; i += 1) {
                const promise = async (index: number) => {
                    const mateId = await BiasContract.tokenOfOwnerByIndex(walletAddress, index);
            ...
            for (const mateId of this.mates) {
                new BMCSItem(this, mateId, mateNames[mateId]).appendTo(this.list);
            }
