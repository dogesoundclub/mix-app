...
export default class LPTokenListenersV2 extends DomNode {
    ...
    private async load() {
        ...
        try {
            ...
            this.append(
                el(".lp-container",
                    el("p", `총 LP: ${CommonUtil.numberWithCommas(utils.formatEther(totalShares))}`),
                    el("hr"),
                    el("p", `추가 APR: +${apr}%`),
                ),
            );
            ...
            this.append(
                el(".lp-container",
                    el("p", `총 LP: ${CommonUtil.numberWithCommas(utils.formatEther(totalShares))}`),
                    el("hr"),
                    el("p", "추가 APR: 가격 정보를 불러오는데 실패했습니다."),
                ),
            );
        }
        ...
        if (walletAddress !== undefined) {
            ...
            this.append(
                el(".my-container",
                    el("p", `내가 넣은 LP: ${CommonUtil.numberWithCommas(utils.formatEther(share))}`),
                    el("hr"),
                    el("p", `내가 가진 LP: ${CommonUtil.numberWithCommas(utils.formatEther(lpBalance))}`),
                ),

                el(".interest-container",
                    el("p", `쌓인 MIX: ${CommonUtil.numberWithCommas(utils.formatEther(mixReward))}`),
                    el("hr"),
                    el("p", `쌓인 KSP: ${CommonUtil.numberWithCommas(utils.formatEther(kspReward))}`),
                    el("hr"),
                    el("p", `쌓인 PUNK: ${CommonUtil.numberWithCommas(utils.formatEther(punkReward))}`),
                ),
                ...
                el("a", "LP 토큰 리스너 등록", {
                    click: () => {
                        new Prompt("LP 토큰 리스너 등록", "얼마만큼의 LP 토큰을 등록하시겠습니까?", "등록하기", async (amount) => {
                ...
                el("a", "LP 토큰 리스너 등록 취소", {
                    click: () => {
                        new Prompt("LP 토큰 리스너 등록", "얼마만큼의 LP 토큰을 등록 취소하시겠습니까?", "등록 취소", async (amount) => {
                            const lp = utils.parseEther(amount);
