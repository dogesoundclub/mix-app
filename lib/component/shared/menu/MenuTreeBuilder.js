      ...
      parent === undefined
        ...
        : [
            (0, skynode_1.el)(
              "li.parent",
              (0, skynode_1.el)(
                `a${location.pathname === `/${parent.uri}` ? ".on" : ""}`,
                parent.name,
                { click: () => ViewUtil_1.default.go(`/${parent.uri}`) }
              )
            ),
          ];
    for (const menuItem of menus) {
      const li = (0, skynode_1.el)(
        "li",
        (0, skynode_1.el)(
          `a${location.pathname === `/${menuItem.uri}` ? ".on" : ""}`,
          menuItem.name,
          { click: () => ViewUtil_1.default.go(`/${menuItem.uri}`) }
        )
      );
