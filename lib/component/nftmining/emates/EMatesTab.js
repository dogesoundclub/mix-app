...
class EMatesTab extends skydapp_browser_1.DomNode {
    constructor() {
        super(".mates-tab");
        this.mates = [];
    ...
    async load() {
        ...
        if (walletAddress !== undefined) {
            ...
            const balance = (await EMatesContract_1.default.balanceOf(walletAddress)).toNumber();
            const promises = [];
            for (let i = 0; i < balance; i += 1) {
                const promise = async (index) => {
                    const mateId = await EMatesContract_1.default.tokenOfOwnerByIndex(walletAddress, index);
            ...
            for (const mateId of this.mates) {
                new EMateItem_1.default(this, mateId, mateNames[mateId]).appendTo(this.list);
            }
        }
        else {
            this.list.empty();
