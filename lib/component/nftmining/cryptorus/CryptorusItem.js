"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const skydapp_browser_1 = require("skydapp-browser");
const ethers_1 = require("ethers");
const CommonUtil_1 = __importDefault(require("../../../CommonUtil"));
const MixContract_1 = __importDefault(require("../../../contracts/mix/MixContract"));
const CryptorusPoolContract_1 = __importDefault(require("../../../contracts/mix/CryptorusPoolContract"));
const CryptorusContract_1 = __importDefault(require("../../../contracts/nft/CryptorusContract"));
const Wallet_1 = __importDefault(require("../../../klaytn/Wallet"));
const KlubsLoader_1 = __importDefault(require("../../../KlubsLoader"));
const Confirm_1 = __importDefault(require("../../shared/dialogue/Confirm"));
class CryptorusItem extends skydapp_browser_1.DomNode {
    constructor(tab, id) {
        super(".cryptorus-item");
        this.tab = tab;
        this.id = id;
        this.claimable = ethers_1.BigNumber.from(0);
        this.append((0, skydapp_browser_1.el)(".content", this.cryptorus = (0, skydapp_browser_1.el)("img", { alt: `cryptorus-${id}` }), (0, skydapp_browser_1.el)(".info", (0, skydapp_browser_1.el)("h5", (0, skydapp_browser_1.msg)("MINING_ITEM_TITLE")), this.mixAmount = (0, skydapp_browser_1.el)(".amount", "Loading...")), (0, skydapp_browser_1.el)(".controller", (0, skydapp_browser_1.el)("button.claim-button", (0, skydapp_browser_1.msg)("MINING_ITEM_BUTTON"), {
            click: async () => {
                if (await Wallet_1.default.connected() !== true) {
                    await Wallet_1.default.connect();
                }
                const owner = await Wallet_1.default.loadAddress();
                if (owner !== undefined) {
                    const balance = await MixContract_1.default.balanceOf(owner);
                    const fee = this.claimable.div(9);
                    if (balance.lt(fee)) {
                        new Confirm_1.default((0, skydapp_browser_1.msg)("MINING_TAB_CONFIRM_TITLE1"), (0, skydapp_browser_1.msg)("MINING_TAB_CONFIRM_DESC1"), (0, skydapp_browser_1.msg)("MINING_TAB_CONFIRM_BUTTON1"), () => {
                            open("https://klayswap.com/exchange/swap?input=0x0000000000000000000000000000000000000000&output=0xdd483a970a7a7fef2b223c3510fac852799a88bf");
                        });
                    }
                    else {
                        if ((await MixContract_1.default.allowance(owner, CryptorusPoolContract_1.default.address)).lt(fee)) {
                            await MixContract_1.default.approve(CryptorusPoolContract_1.default.address, ethers_1.constants.MaxUint256);
                            setTimeout(async () => {
                                await CryptorusPoolContract_1.default.claim([this.id]);
                            }, 2000);
                        }
                        else {
                            await CryptorusPoolContract_1.default.claim([this.id]);
                        }
                    }
                }
            },
        }))));
        this.loadImage();
        this.load();
        this.refreshInterval = setInterval(() => this.load(), 1000);
    }
    async loadImage() {
        const metadata = await KlubsLoader_1.default.loadMetadata(CryptorusContract_1.default.address, this.id);
        this.cryptorus.domElement.src = `${metadata.image}`;
    }
    async load() {
        const claimable = await CryptorusPoolContract_1.default.claimableOf(this.id);
        if (this.deleted !== true) {
            this.mixAmount.empty().appendText(CommonUtil_1.default.numberWithCommas(ethers_1.utils.formatEther(claimable), 5));
            this.tab.changeMix(claimable.sub(this.claimable));
            this.claimable = claimable;
        }
    }
    delete() {
        clearInterval(this.refreshInterval);
        super.delete();
    }
}
exports.default = CryptorusItem;
//# sourceMappingURL=CryptorusItem.js.map