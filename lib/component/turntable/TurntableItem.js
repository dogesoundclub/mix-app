...
class TurntableItem extends skydapp_browser_1.DomNode {
    constructor(id, currentBlock, info, showMix) {
        super(".turntable-item");
        this.id = id;
        const lifetime = info.endBlock - currentBlock;
        this.append((0, skydapp_browser_1.el)("img", { src: turntables_json_1.default[info.typeId].img }), this.nameDisplay = (0, skydapp_browser_1.el)("h4", `턴테이블 #${id}`), (0, skydapp_browser_1.el)(".container", this.gradeDisplay = (0, skydapp_browser_1.el)("span.grade", `${turntables_json_1.default[info.typeId].name}`, { style: { color: turntables_json_1.default[info.typeId].color } }), (0, skydapp_browser_1.el)("hr"), (0, skydapp_browser_1.el)("span", `소유자: ${CommonUtil_1.default.shortenAddress(info.owner)}`)), showMix !== true ? undefined : this.claimableDisplay = (0, skydapp_browser_1.el)(".mix", "Loading..."), (0, skydapp_browser_1.el)(".lifetime", `Lifetime: ${CommonUtil_1.default.numberWithCommas(String(lifetime < 0 ? 0 : lifetime))} Blocks`));
        if (showMix === true) {
            this.loadClaimable();
        }
